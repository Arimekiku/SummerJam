...
public class CameraHandler : MonoBehaviour
{
    ...
    public void SwapCamera(CinemachineVirtualCamera firstSideCamera, CinemachineVirtualCamera secondSideCamera, Vector2 direction)
    {
        if (currentCamera == firstSideCamera && (direction.x > 0f || direction.y > 0f))
        {
            currentCamera.enabled = false;

            currentCamera = secondSideCamera;
        } 
        else if (currentCamera == secondSideCamera && (direction.x < 0f || direction.y < 0f))
        {
            currentCamera.enabled = false;
